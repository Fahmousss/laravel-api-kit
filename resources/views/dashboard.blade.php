...
<div class="max-w-2xl mx-auto mt-16 text-center">
    <h1 class="text-3xl font-bold mb-3 text-emerald-950">Welcome to your Dashboard, {{ $authenticatedUser->displayName }}</h1>
    
    <p class="text-slate-500 mb-1.5">Signed in as <strong class="text-slate-800 font-medium">{{ $authenticatedUser->email }}</strong></p>
    <p class="text-slate-500 mb-4">Member since {{ $authenticatedUser->memberSince }}</p>
    
    @unless($authenticatedUser->isEmailVerified)
